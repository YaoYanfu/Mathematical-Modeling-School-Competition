...
DATA_PATH = ROOT / "outputs" / "问题二" / "problem2_trajectory_10hz_kalman.csv"
OUT_3D = ROOT / "outputs" / "问题二" / "problem2_trajectory_10hz_kalman_3d.png"
OUT_PROJECTION = ROOT / "outputs" / "问题二" / "problem2_trajectory_10hz_kalman_projection.png"
...
axes[0].set_title("X-Y投影")
axes[0].axis("equal")
axes[0].grid(alpha=0.3)

axes[1].plot(df["time_s"], df["x_kalman_m"], "r-", lw=1.2)
axes[1].set_xlabel("时间 (s)")
axes[1].set_ylabel("X坐标 (m)")
axes[1].set_title("X-时间投影")
axes[1].grid(alpha=0.3)

axes[2].plot(df["time_s"], df["y_kalman_m"], "g-", lw=1.2)
axes[2].set_xlabel("时间 (s)")
axes[2].set_ylabel("Y坐标 (m)")
axes[2].set_title("Y-时间投影")
