...
preprocess_dir = next(
    (
        ...
        if (path / 'problem1_method1_10hz_device_time.csv').exists()
        and (path / 'problem1_method2_10hz_device_time.csv').exists()
    ),
    preprocess_candidates[0],
)
df1 = pd.read_csv(preprocess_dir / 'problem1_method1_10hz_device_time.csv')
df2 = pd.read_csv(preprocess_dir / 'problem1_method2_10hz_device_time.csv')
...
axes[0, 2].set_xlabel('X坐标 (m)')
axes[0, 2].set_ylabel('Y坐标 (m)')
axes[0, 2].set_title('方式1 (4Hz) X-Y轨迹')
...
axes[1, 2].set_title('方式2 (5Hz) X-Y轨迹')
